...
multiples [] _ = []
multiples matrix n = let 
                        arr = map (\x -> filter (\p -> mod p n == 0) x) matrix
                        in concat arr
-- === mfuse ===
...
helperSelect :: Entry -> [Char]
helperSelect (Entry _ y) = y

select :: [[Entry]] -> [Char] -> ([Char], [[Char]])
select [] key = (key,[])
select entries key = let
                     lst =  map (\e -> filter (\(Entry x y) -> x == key)e) entries
                     in (key, map (\p-> helperSelect p) (concat lst))
-- === sold ===
