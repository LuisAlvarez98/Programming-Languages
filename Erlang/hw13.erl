-module(hw13).
-import(lists, [map/2, filter/2, zip/2, foldl/3]). % To avoid uning "lists:" on every function call.
-import(math, [pow/2]).
-export([start/0]).
...
firstn([H | T], N) -> if
	N >= 0 -> [H | firstn(T, N - 1)];
	true -> io:format("Not implemented for negative nums")
	end.
...
even([]) -> [];
even(L) -> filter(fun(P) -> P rem 2 == 0 end, L).

% === reverse ===

reverse([]) -> [];
reverse([H | T]) when is_list(H) -> reverse(T) ++ [reverse(H)];
reverse([H | T]) when not is_list(H) -> reverse(T) ++ [H].

% === maskedSum ===

getMaskedList([],[]) -> [];
getMaskedList([H1 | T1], [H2 | T2]) -> filter(fun(X)-> tl(X) end,[[H1 | H2] | getMaskedList(T1, T2)]).

maskedSum([], []) -> 0;
maskedSum([H1 | T1], [H2 | T2]) -> foldl(fun(X, Sum) -> X + Sum end, 0, map(fun(Y)-> hd(Y)end ,getMaskedList([H1 | T1], [H2 | T2]))).
% === evaluate ===
 
evaluate([], _) -> 0;
evaluate([H | T], X) -> H * pow(X, length([H | T]) - 1) + evaluate(T, X).
...
start() -> 
	...
	io:format("=== lastn ===~n"),
	io:format("~p~n", [lastn(['a', 'b', 'c', 'd', 'e'], 3)]), % [c,d,e]
	io:format("~p~n", [lastn([10, 20, 30, 40, 50], 5)]), % [10,20,30,40,50]
	io:format("=== sum ===~n"),
