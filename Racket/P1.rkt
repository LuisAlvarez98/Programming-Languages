...
(define (exam.stdev lst)
  (if (null? lst)
        0
        (sqrt(/(apply +(map (lambda (x) (expt (- x (getMean lst)) 2)) lst)) (-(length lst)1)))  
   )
)
...
;; === exam.toBinary ===
(define (helperBinary x)
  (cond
    ((= x 0) (list 0))
    ((= x 1) (list 1))
    ((append (exam.toBinary (quotient x 2)) (list (modulo x 2))))
   )
 )
...
(define (scanHelper lst)
 (if (null? lst)
       #f ;;if the list is empty but never ended then #f
       (if (equal? (car lst) 2)
           #t ;; if 2 then it means it ended
           (scanHelper (cdr lst)) ;; all the 0 and 1s
        )
   )
 )
...
(define (getPos cols)
   (cond
     ((null? cols) '())
     ((equal? (car cols) "id") (cons 0 (getPos (cdr cols))))
     ((equal? (car cols) "g1") (cons 1 (getPos (cdr cols))))
     ((equal? (car cols) "g2") (cons 2 (getPos (cdr cols))))
     ((equal? (car cols) "g3") (cons 3 (getPos (cdr cols))))
    )
)
...
(define (helperSelect indices table)
  (if (null? indices)
      '()
     (cons(map (lambda (x)  (access x (car indices)) ) table) (helperSelect (cdr indices) table))
  )
)
(define (helperTwo lst)
  (if (equal?(length lst)1)
      lst
      (map list (car lst) (cadr lst))
  )
)
(define (exam.select table cols)
  (cons cols(helperTwo(helperSelect (getPos cols) (cdr table))))
)
